...
class Functions:
    ...
    async def insert_data(self, category, numb):
        async with self.connection.pool.acquire() as conn:
                ...
                try:
                    ...
                    while numb != 0:
                        try:
                            ...
                            if await cursor.execute(f"SELECT title FROM {category} where {category}_id = '{data[f'{category}_id']}';"):
                            ...
                            print("Inner inserting error", ex)
                    driver.close()
                except Exception as ex:
                    print(ex, "insert")
        ...
        async with self.connection.pool.acquire() as conn:
            async with conn.cursor() as cursor:
                ...
                try:
                    ...
                    end_getting_data = start_getting_data + numb
                    while start_getting_data < end_getting_data:
                        try:
                            cur = {}
                            # проверка на наличие записи в таблице с нужным id
                            await cursor.execute(f"select {category}_id from {category};")
                            ids = [info[0] for info in await cursor.fetchall()]
                            if start_getting_data <= len(ids):
                                # запись есть => получение данных из нее в row
                                await cursor.execute(f"select * from {category} where {category}_id = '{ids[start_getting_data-1]}'")
                                row = list(await cursor.fetchone())
                                # декодирование первой фотографии объявления
                                row[2] = base64.decodebytes(row[2])
                            ...
                            # и необходимо дополнить ее, используя ф-ю insert_data
                                ...
                                print(f"Need to insert {end_getting_data - start_getting_data} more ads")
                                await self.insert_data(category, end_getting_data - start_getting_data)
                        except Exception as ex:
                            print("Inner getting error", ex)
                    return result
                except Exception as ex:
                    print(ex, "get")
